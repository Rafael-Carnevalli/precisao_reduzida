...
__global__ void assignment_kernel(const __half *X, const __half *C, int *assign, float *sse_sum, int N, int K) {
    ...
    int best = -1;
    __half bestd = __float2half(FLT_MAX);
    for (int c = 0; c < K; c++) {
        __half diff = X[i] - C[c];
        __half d = diff * diff;
        if (d < bestd) {
    ...
    assign[i] = best;
    atomicAdd(sse_sum, __half2float(bestd));
}
...
int main(int argc, char **argv){
    ...
    clock_t t0 = clock();
    int iters = 0; __half sse = __float2half(0.0f);
